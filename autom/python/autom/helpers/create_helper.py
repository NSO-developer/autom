...
def dryrun_configuration_xml(conf_type, cdb_config_file, diff_file_xml, uinfo):
    """
    ...
    append_to_file(diff_file_xml, """</data>""")
    #except Exception:
    #    result = None
def dryrun_configuration_cli(conf_type, cdb_config_file, diff_file, uinfo, logger):
    """
    Open a write transaction towards the CDB with uinfo file
    and execute a commit dry-run (outformat xml) of the configuration
    and write it to diff_file_xml (which is a filename)
    ...
    #try:
    commit_params = ncs.maapi.CommitParams()
    commit_params.dry_run_cli_c()
    result = trans.apply_params(params=commit_params)

    logger.info('Result:', str(result))
    try:
        if result['local-node']:
                write_file(diff_file, result['local-node'])
    except Exception:
        logger.info("CLI output of device diff is missing, cli-c file has not been written")

def dryrun_configuration_native(conf_type, cdb_config_file, diff_file, uinfo, logger):
    """
    Open a write transaction towards the CDB with uinfo file
    and execute a commit dry-run (outformat xml) of the configuration
    and write it to diff_file_xml (which is a filename)
    ...
    try:
        if result['device']:
            for device, value in result['device'].items():
                cli_diff = value
                write_file(diff_file, cli_diff)
    except Exception:
        logger.info("CLI output of device diff is missing, cli-zfile has not been written")
...
def plan_status_reached(logger, uinfo, plan_location):
...
def find_xpath_for_keypath(logger, keypath, services_xpath):
    return services_xpath.get(str(keypath))

def wait_for_zombie(logger, max_time_to_wait, sleep_time, root, trans, plan_location, xpath_node):
    time_spent = 0
    while time_spent < max_time_to_wait:
        time_spent +=1
        if zombie_exists(logger, root, trans, plan_location, xpath_node):
            time.sleep(sleep_time)
            continue
        else:
            break
    logger.info("Time spent waiting for zombie:", time_spent)

def nano_service_ready(logger, uinfo, trans, plan_location, sock_maapi, max_time_to_wait, sleep_time):
    """
    ...
    plan_reached = plan_status_reached(logger, uinfo, plan_location_node)
    retries = max_time_to_wait / sleep_time
    iterations = 1
    while plan_reached is False:
        iterations += 1
        logger.info("Inside loop of plan_reached, waiting for 0.2s if plan_reached is ", plan_reached)
        time.sleep(sleep_time)
        plan_reached = plan_status_reached(logger, uinfo, plan_location_node)
        if plan_reached:
            return True
...
def get_parent(logger, sock_maapi, trans, path, top_level_services, parent_services):
    """
...
def get_top_level_parent(logger, sock_maapi, trans, path, top_level_services, parent_services):
    """
    ...
    None is returned.
    """
    is_top_level, keypath = get_parent(logger, sock_maapi,
                                            trans,
                                            path,
                                            top_level_services,
                                            parent_services)
    if is_top_level:
        ...
        return get_top_level_parent(logger, sock_maapi, trans, keypath, top_level_services, parent_services)
    else:
        return None

def get_specific_check_sync_result(logger, trans, service_instances):
    """
    ...
    for item in service_instances:
        ...
        logger.info("Check-sync of kp_node path: %s" %kp_node._path)
        logger.info("Result: %s" % str(check_sync_result.in_sync))
        xpath_str = ''
        if 'True' in str(check_sync_result.in_sync):
            xpath_str = xpath(kp_node)
            pair = {kp_node._path:xpath_str}
            services_list.update(pair)
            logger.info("Appended %s" %pair)
    return services_list

def get_service_keypaths(logger, uinfo,
    ...
    logger.info("Regular_services: %s" % regular_services)
    logger.info("Parent_services: %s" % parent_services)
    logger.info("Child_services: %s" % list(unique_child_services))
    logger.info("Top_level_services: %s" % top_level_services)

    return list(unique_child_services), parent_services, regular_services, top_level_services, services_xpath

def cdb_config_capture(logger, sock_maapi, thandle, output_folder, phase,
                       config_types, kp_input, index):
    """
    ...
    if 'pre_config_xpath_xml' in config_types:
        ...
        save_configuration(sock_maapi, thandle, maapi.CONFIG_XML_PRETTY,
                            kp_input, pre_config_file_xml, logger, "w")
    ...
    if 'service_config_xml' in config_types:
        ...
        save_configuration(sock_maapi, thandle, maapi.CONFIG_XML_PRETTY,
                            kp_input, service_config_file_xml, logger, "w")
        logger.info(f'Iter {iter} ============================= Writing file {service_config_file_xml}')
    ...
    if 'cli' in config_types:
        ...
        save_configuration(sock_maapi, thandle, maapi.CONFIG_C_IOS + maapi.CONFIG_WITH_SERVICE_META, '/',
                            config_file_cli, logger, "w")
        config_cli_cleanup(config_file_cli, config_file_clean_cli)
        os.remove(config_file_cli)

def device_config_capture(logger, sock_maapi, trans, thandle, device,
...
def capture_modifications(trans, username, keypath, flag,
                                 get_modifications_file_xml, write_to_file):
    """
    ...
    keypath_node = ncs.maagic.get_node(trans, keypath)
    ...
    try:
        ...
        if write_to_file:
            write_file(get_modifications_file_xml,
        ...
        with ncs.maapi.Maapi() as m, ncs.maapi.Session(
            ...
            if write_to_file:
                ...
                return '<data>' + diff_output.result_xml.local_node.data + '</data>'
def capture_modifications_cli(trans, username, keypath,
                                 get_modif_file_cli, write_to_file):
    """
    Captures device modifications based on service keypath
    Sets services global-settings collect-forward-diff to true if not
        already set!
    ...
    keypath_node = ncs.maagic.get_node(trans, keypath)
    diff_input = keypath_node.get_modifications.get_input()
    diff_input.outformat = 'cli-c'
    try:
        diff_output = keypath_node.get_modifications(diff_input)
        if write_to_file:
            write_file(get_modif_file_cli,
                    diff_output.cli_c.local_node.data)
        else:
            return diff_output.cli_c.local_node.data
...
def zombie_exists(logger, t, root, path_node, str_xpath):
    logger.info("Zombie path_node:", path_node._path)
    node = ncs.maagic.get_node(t, path_node._path)
    zombie = f'/zombies/service[service-path="{strip_xpath_prefixes(xpath(node))}"]'
    logger.info("Inside loop looking for the zombie at ", zombie)
    if xpath_kp(logger, root, zombie) is not None:
        logger.info("Zombie found!")
        return True
    else:
        return False
...
def load_cdb_config_from_file_and_leave_trans_open(sock_maapi, load_file, no_networking, uinfo):
    try:
        thandle_rw = maapi.start_trans2(sock_maapi, _ncs.RUNNING,
                                           _ncs.READ_WRITE, uinfo.usid)
...
def service_has_plan(logger, kpath, xpath, uinfo):
    trans, thandle, sock_maapi, root = _open_new_trans(uinfo)
    logger.info("Kpath = ", kpath)
    kp_node = ncs.maagic.get_node(trans, kpath)
    #kp_node_oper_data = ncs.maagic.cd(root, kpath)
    plan_location = get_plan_location(logger, root, kp_node, xpath)
    if plan_location is not None:
        plan_location_node = ncs.maagic.get_node(trans, maapi.xpath2kpath(
                                        sock_maapi,
                                        plan_location))
        logger.info("Plan Location Node: ", plan_location_node._path)
        _close_trans(trans)
...
def get_children(logger, sock_maapi, trans, path):
    """
    Get children returns a list of child services to a given path (if found)
    otherwise the list [] returned will be empty
    """
...
def has_children(logger, sock_maapi, trans, path):
    """
    Has children returns a boolean True if child services exist to a given
    path (if found) otherwise the False is returned
    """
...
def get_parent(logger, uinfo, trans, path, services_list):
    """
    Get Parent returns the boolean True for a top level service, False if
    a normal parent (mid layer service) together with the parent keypath
    """
    child_services, parent_services, regular_services, top_level_services, services_xpath = get_service_keypaths(logger,
                            uinfo,
                            services_list,
                            [])
    for keypath in parent_services:
...
def get_top_level_parent(logger, uinfo, trans, path, services_list):
    ...
    None is returned.
    """
    is_top_level, keypath = get_parent(logger, uinfo,
                                            trans,
                                            path,
                                            services_list)
    if is_top_level:
...
def is_parent(logger, uinfo, trans, path, services_list):
    """
...
def is_top_level_parent(logger, uinfo, trans, path, services_list):
    """
